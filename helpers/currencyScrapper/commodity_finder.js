...
const getAllCommodities = asyncErrorWrapper(async (req, res, next) => {
  ...
  for (let i = 1; i < table.length + 1; i++) {
    const name = $(
      `#cross_rate_1 > tbody > tr:nth-child(${i}) > td:nth-child(2)`
    )
      .text()
      .trim();
    const month = $(
      ...
      .trim();
    const last = $(
      `#cross_rate_1 > tbody > tr:nth-child(${i}) > td:nth-child(4)`
    )
      .text()
      .trim();
    const high = $(
      `#cross_rate_1 > tbody > tr:nth-child(${i}) > td:nth-child(5)`
    )
      .text()
      .trim();
    const low = $(
      `#cross_rate_1 > tbody > tr:nth-child(${i}) > td:nth-child(6)`
    )
      .text()
      .trim();
    const chg = $(
      `#cross_rate_1 > tbody > tr:nth-child(${i}) > td:nth-child(7)`
    )
      .text()
      .trim();
    const chgPerc = $(
      ...
      .trim();
    const time = $(
      `#cross_rate_1 > tbody > tr:nth-child(${i}) > td:nth-child(9)`
    )
      .text()
      .trim();
    allCommodities.push({
...
const getCommAdvices = asyncErrorWrapper(async (req, res, next) => {
  ...
  for (let i of [1, 2, 3, 4, 5, 6, 7]) {
    const name = $(
      `#QBS_5_inner > tbody > tr:nth-child(${i}) > td:nth-child(2)`
    )
      .text()
      .trim();
    const last = $(
      `#QBS_5_inner > tbody > tr:nth-child(${i}) > td:nth-child(3)`
    )
      .text()
      .trim();
    const chg = $(`#QBS_5_inner > tbody > tr:nth-child(${i}) > td:nth-child(4)`)
      .text()
      .trim();
    const chgPerc = $(
